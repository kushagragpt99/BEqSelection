...
#ludfun <- function(state) {
    ...
    #p3 = sum(dmvnorm(t(del_X - f[, - (N + 1)] * del_t), sigma = Sigma * del_t, log = TRUE))
    ### add inverse gamma
...
ludfun <- function(state) {
    ...
    inv_Sig = solve(Sigma)
    for (k in 1:N) {
        del_X = matrix(X_n[, k + 1] - X_n[, k], nrow = N.l96, ncol = 1) # try using diff() function
    ...
    p3 = dnorm(theta, mean = alpha, sd = beta, log = TRUE)
...
MH <- function(n, init) {
    ...
    scale <- rep(.0001, n.param) #
    scale[(n.X + 1):(n.X + n.theta)] <- .08 #0.08
    scale[(n.X + n.theta + 1):(n.param)] <- .03  # 0.04
    accept.prob = 0
    state = init
