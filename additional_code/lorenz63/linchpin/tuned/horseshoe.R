...
ludfun <- function(state) {
    ...
    lambda = state[n.X + n.theta + 1]
    tau = state[n.X + n.theta + 2]
    ...
    p2 = (-1 / 2) * sum((B_vec) ^ 2) / (sigma^2* lambda^2 * tau^2) - log(1 + lambda^2) - log(1 + tau^2)
...
linchpin <- function(n, init, scale_vec) {
    ...
    scale = rep(0.0005 * 1, n.X + n.theta + 2)
    ...
    scale[(n.X + n.theta + 1):(n.X + n.theta + 2)] = 0.1
    ...
    for (i in 1:n) {
        ...
        param_mat[i, (n.theta + n.sigma + 1):(n.sigma + n.theta + 2)] = state[(n.X + n.theta + 1):(n.X + n.theta + 2)]
...
init[(n.X + n.theta + 1):(n.X + n.theta + 2)] = 4

sigma = sqrt( mean( diag( var(t(Y)) ) ) )
