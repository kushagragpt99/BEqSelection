...
linchpin <- function(n, init) {
    ...
    scale = rep(0.01, n.X + n.theta) # 0.01
    scale[(n.X + 1):(n.X + n.theta)] = 2 # 1.8
    accept.prob = 0
    state = init
...
load('ou_linch_spike_truth_T2_1e5_try')
# hyper-parameters
...
pm = ans[[1]]
colMeans(pm)
pdf(file = 'inf_linch_tune.pdf')
plot.ts(pm)
dev.off()
save(ans, file = "OU_linch_1e6_T2_fin")
# 11:30

#pdf(file = 'final_trace_10th.pdf')
#plot.ts(ans[[1]][seq(1,1e6,10),])
#dev.off()